...
function SeguimientoContent() {
  ...
  async function fetchData() {
    ...
    setError(null);
    try {
      const res = await fetch(`${API}/registration/tracking/${eeId}?t=${encodeURIComponent(token)}`);
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch(`${API}/registration/tracking/${eeId}/receipt?t=${encodeURIComponent(token)}`, {
        method: 'POST',
