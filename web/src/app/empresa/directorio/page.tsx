...
function PerfilModal({ empresa, esEncargado, onClose }: { empresa: any; esEncargado: boolean; onClose: () => void }) {
    ...
    <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {esEncargado && (
            <Link
              href={`/empresa/solicitudes?nueva=1&receptoraId=${empresa.empresaeventoId}&receptoraNombre=${encodeURIComponent(empresa.nombre)}`}
...
export default function DirectorioPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      <PerfilModal empresa={perfilAbierto} esEncargado={esEncargado} onClose={() => setPerfilAbierto(null)} />

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">Directorio de participantes</h1>
          <p className="text-sm text-gray-400 mt-0.5">Consulta el perfil de las empresas antes de solicitar una reunión</p>
        </div>
          ...
          {filtradas.map((em: any) => (
            ...
            >
              <div className="p-5">
                ...
                <p className="mt-3 text-xs font-bold text-[#449D3A]">Ver perfil completo →</p>
              </div>
            </button>
          ))}
        </div>
