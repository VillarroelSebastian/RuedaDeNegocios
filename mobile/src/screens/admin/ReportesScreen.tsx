...
export default function ReportesScreen() {
  ...
  const visibles = useMemo(() => {
    ...
    return [...base].sort((a, b) => orden === 'estrellas' ? Number(b.EstrellasDadas) - Number(a.EstrellasDadas) : orden === 'dinero' ? Number(b.DineroGeneradoAproxUSD) - Number(a.DineroGeneradoAproxUSD) : Number(b.Reuniones) - Number(a.Reuniones));
  ...
  return <View style={{ flex: 1, backgroundColor: '#f8fafc' }}>
    ...
    {loading ? <ActivityIndicator color={GREEN} style={{ marginTop: 45 }} /> : <ScrollView contentContainerStyle={{ padding: 14, gap: 10 }}>{visibles.length === 0 ? <Text style={{ textAlign: 'center', color: '#94a3b8', marginTop: 40 }}>No hay datos para este reporte.</Text> : visibles.map((fila, i) => <View key={i} style={{ backgroundColor: '#fff', borderRadius: 14, borderWidth: 1, borderColor: '#e2e8f0', padding: 13 }}>{columnas.map((c) => <View key={c} style={{ flexDirection: 'row', alignItems: 'flex-start', paddingVertical: 4 }}><Text style={{ width: '42%', fontSize: 10, fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase' }}>{c.replace(/([A-Z])/g, ' $1')}</Text><Text selectable style={{ flex: 1, fontSize: 12, color: '#334155', fontWeight: '600' }}>{String(fila[c] ?? '—')}</Text></View>)}</View>)}</ScrollView>}
  </View>;
}
