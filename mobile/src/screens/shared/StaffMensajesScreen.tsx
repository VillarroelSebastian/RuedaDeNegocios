...
export default function StaffMensajesScreen() {
  ...
  const cargarConvs = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/staff/mensajes/conversaciones`);
  ...
  const cargarMensajes = useCallback(async (eeId: number) => {
    try {
      const res = await fetch(`${API_URL}/staff/mensajes?eeId=${eeId}`);
      const data = res.ok ? await res.json() : [];
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={s.emptyWrap}>
            ...
            <Text style={s.emptyText}>Aún no se ha enviado ningún mensaje.</Text>
            <Text style={s.emptySub}>Usa "Nuevo" para escribirle a una empresa.</Text>
          </View>
        }
        renderItem={({ item }: any) => (
          <TouchableOpacity
        ...
        <View style={s.overlay}>
          <View style={s.sheet}>
            <View style={s.sheetHeader}>
              <Text style={s.sheetTitle}>Nuevo mensaje</Text>
            ...
            {cargandoEmp ? (
              ...
              <FlatList
                data={filtradas}
                keyExtractor={(e: any) => String(e.eeId)}
                style={{ maxHeight: 380 }}
                contentContainerStyle={{ padding: 10 }}
                ListEmptyComponent={<Text style={[s.emptyText, { paddingVertical: 20 }]}>Sin empresas habilitadas.</Text>}
                renderItem={({ item }: any) => (
                  <TouchableOpacity
                    style={s.empRow}
                    onPress={() => { setModalNueva(false); setActiva({ eeId: item.eeId, nombre: item.nombre }); }}
                    activeOpacity={0.8}
                  >
