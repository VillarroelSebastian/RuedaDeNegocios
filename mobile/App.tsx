...
export default function App() {
  ...
  useEffect(() => {
    ...
    console.log('[App] Session restore starting...');
    ...
    // Safety timeout: if AsyncStorage never responds, show Login after 6s
    const timer = setTimeout(() => {
      console.warn('[App] Session restore timed out (6s) — clearing and showing Login');
      userStore.clear();
      finish('Login');
    }, 6000);
    ...
    userStore.load()
      .then((user) => {
        ...
        // Validate stored session has required fields and a known role
        if (user?.id && role && VALID_ROLES.includes(role)) {
          ...
          if (user) {
            console.warn('[App] Stored session invalid (missing id or unknown role:', role, ') — clearing');
            userStore.clear();
          } else {
            console.log('[App] No stored session — showing Login');
          }
          finish('Login');
        }
      })
